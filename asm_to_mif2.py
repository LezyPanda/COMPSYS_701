...
def assemble_instruction(parts, labels, pc):
    ...
    if mnemonic in ['NOOP', 'END', 'ENDPROG']:
        ...
        if mnemonic in ['JMP','PRESENT','SZ','STRPC','MAX']:
            am = 'immediate'
        else:
        ...
        if am == 'register':
            ...
            operand = int(core) if core.isdigit() else labels.get(core, 0)
        elif am == 'direct':
            key = ops[0].lstrip('$')
        ...
        if am == 'immediate':
            ...
            key = ops[2][1:]
            operand = int(key) if key.isdigit() else labels.get(key, 0)
    if mnemonic == 'DATACALL' and am == 'immediate':
        opcode = opcodes['DATACALL2']
...
#this is not poggers
def instr_length(line):
    parts = [p for p in re.split(r'[\s,]+', line.strip()) if p]
    if not parts:
        return 0
    m = parts[0].upper()
    ops = parts[1:]
    if m in ['NOOP','END','ENDPROG','CLFZ','CER','CEOT','SEOT','LER','SSVOP','SSOP','LSIP']:
        return 1
    if len(ops) == 3:
        mode = detect_addr_mode(ops[2])
        return 2 if mode in ['immediate','direct'] else 1

    if len(ops) in [1,2] and m in ['JMP','PRESENT','SZ','STRPC','MAX']:
        return 2
